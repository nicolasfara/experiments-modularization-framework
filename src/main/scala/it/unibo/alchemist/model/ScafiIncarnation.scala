...
import it.unibo.alchemist.model.implementations.actions.{RunMacroScafiProgram, SendMacroScafiMessage}
...
sealed class ScafiIncarnation[T, P <: Position[P]] extends Incarnation[T, P]{
  ...
  override def createAction(
    ...
    if(param=="send") {
      val alreadyDone = ScafiIncarnationUtils.allActions[T,P,SendMacroScafiMessage[T,P]](node, classOf[SendMacroScafiMessage[T,P]]).map(_.program)
      val spList = ScafiIncarnationUtils.allScafiProgramsFor[T,P](node)
      spList --= alreadyDone
      if (spList.isEmpty) {
        throw new IllegalStateException("There is no program requiring a " + classOf[SendMacroScafiMessage[T,P]].getSimpleName + " action")
      }
      if (spList.size > 1) {
        throw new IllegalStateException("There are too many programs requiring a " + classOf[SendMacroScafiMessage[T,P]].getName + " action: " + spList)
      }
       new SendMacroScafiMessage[T,P](env, scafiNode, reaction, spList.head)
    } else {
      new RunMacroScafiProgram[T, P](
  ...
  override def createCondition(rand: RandomGenerator, env: Environment[T, P] , node: Node[T], time: TimeDistribution[T], reaction: Reaction[T], param: String): Condition[T] = {
    ...
    val alreadyDone = ScafiIncarnationUtils
      ...
      .collect { case x: RunMacroScafiProgram[T,P] => x }
    val spList: mutable.Buffer[RunMacroScafiProgram[T,P]] = ScafiIncarnationUtils.allScafiProgramsFor(node)
    spList --= alreadyDone
    if (spList.isEmpty) {
...
object ScafiIncarnationUtils {
  ...
  def allScafiProgramsFor[T,P<:Position[P]](node: Node[T]) =
    allActions[T,P,RunMacroScafiProgram[T,P]](node, classOf[RunMacroScafiProgram[T,P]])
